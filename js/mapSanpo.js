function mapSanpo(mapId, map_container) {
...
//                layers: [t_pale]
        ...
        //$(function () {
        //    var mapId = 'map_sanpo';
        //    const sidebarId = 'sidebar_sanpo';
        //    const map = map_sanpo;
        var t_map = { map_1851_ymkz: "1851_横浜村並近傍之図", map_1859_ykjz: "1859_横浜開港地割之図", map_1860_tyf: "1860_東海道名所之内横浜風景", map_1860_ymi: "1860_横浜名所一覧（海岸）", map_1861_gkyz: "1861_御開港横濱之図", map_1861_syf: "1861_再改横浜風景", map_1865_gkyzz: "1865_御開港横浜之全図", map_1868_ymzz: 3, map_now: "現在", map_sanpo: "散歩" };
        var z_map = { map_1851_ymkz: 2, map_1859_ykjz: 3, map_1860_tyf: 7, map_1860_ymi: 7, map_1861_gkyz: 2, map_1861_syf: 4, map_1865_gkyzz: 3, map_1868_ymzz: 3, map_now: 16, map_sanpo: 16 };
        ...
        function showSidebar(e, feature) {
            var displayText = "<h3>" + feature.properties.name + "</h3>";

            var spotInfos = feature.properties.spotInfos;
            for (let i = 0; i < spotInfos.length; i++) {
                let spotInfo = spotInfos[i];
                if (!spotInfo.title == (null | "")){displayText += ("<h4>" + spotInfo.title + "</h4>");}
                if (!spotInfo.URL == (null | "")){displayText += (spotInfo.URL + "<br>");}
                var author_year = "";
                if (!spotInfo.author == (null | "")){author_year += spotInfo.author;}
                if (!spotInfo.publication_year == (null | "")){author_year += "【" + spotInfo.publication_year + "】<br>";}
                displayText += author_year;
                if (!spotInfo.volume == (null | "")){displayText += (spotInfo.volume + "<br>");}
                if (!spotInfo.source == (null | "")){displayText += (spotInfo.source + "<br><br>");}
                if (!spotInfo.description == (null | "")){displayText += (spotInfo.description + "<br><br>");}
            }
            document.getElementById(sidebarId).innerHTML = displayText;
            sidebar.show();
        }
        ...
        function sanpoOverlay(data, marker, layerName) {
            var sanpo_geoj = L.geoJSON(data, {
                style: function (feature) { return feature.properties.style },
                onEachFeature: function (feature, layer) {
                    layer.on('mouseover', function (e) {
                        var breaks = feature.properties.description.length > 0 ? "<br><br>" : "";
            ...
            sanpo_geoj.addTo(map1)
            control.addOverlay(sanpo_geoj, layerName); // Add the layer to the Layer Control.
        }


        var layerName_5 = "<span style=\"display: inline-block; width: 85%; color:black; background-color: rgb(201, 233, 183);\">関内コース</span>";
        var sanpoMarker = L.divIcon({ className: 'triangle_sanpo' });
        $.getJSON("./gj/sanpo_kannai.geojson", function (data) {
            sanpoOverlay(data, sanpoMarker, layerName_5)
        });

        var control = L.control.layers(Map_b2); // Grab the handle of the Layer Control, it will be easier to find.
        control.addTo(map1);

        L.control.scale({ maxWidth: 250, imperial: false }).addTo(map1);
    //});
    }
...
//                layers: [t_pale]
        ...
        L.control.scale({ maxWidth: 250, imperial: false }).addTo(map2);
    //});
    }
}
